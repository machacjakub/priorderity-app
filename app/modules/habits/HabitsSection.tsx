...
export const HabitsSection = () => {
	...
	return (
		<div className='fixed bottom-4 right-4 flex flex-col gap-2'>
			{!isHidden.value && habits.map( habit => <HabitStreakTag key={habit.label} label={habit.label} streaks={getStreak( habit, doneActivities )}/> )}
			<div className="flex gap-2 items-center">
				...
				<button onClick={isHidden.toggle}>{isHidden.value ? <EyeOutlined/> : <EyeSlashOutlined/>}</button>
			</div>
		</div>
	);
};