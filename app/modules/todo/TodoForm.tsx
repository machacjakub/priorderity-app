	...
	return (
		<ModalWindow onClose={onClose} isOpen={isOpen} tailwind="flex items-center justify-center">
			<div className="text-foreground p-6 backdrop-blur-lg bg-background/50 p-4 rounded-2xl border-2 border-foreground/90 " onClick={( e ) => e.stopPropagation()}>
				...
				<FadingLine/>
				<form className='mt-4 grid grid-cols-4'>
					<label className="text-right p-2">Title: </label> <input placeholder="title of the todo" className="text-background/80 px-2 m-1 col-span-2"/> <div/>
					<label className="text-right p-2">Priority: </label> <input placeholder="0" type='number' className="text-background/80 px-2 m-1 w-20 col-span-2" /> <div/>
				...
				<div className='flex justify-between'>
					...
					<button onClick={onClose} className="bg-gradient-to-r px-3 py-2 from-violet-500/50 to-cyan-400/30 p-2 mt-6 rounded-lg text-foreground hover:from-violet-500/75 hover:to-cyan-400/50" >Submit</button>
				</div>
			</div>
		</ModalWindow>
	);
