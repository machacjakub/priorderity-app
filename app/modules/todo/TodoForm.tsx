			...
			<div className="text-foreground p-6 backdrop-blur-lg bg-background/50 p-4 rounded-2xl border-2 border-foreground/90 animate-in" onClick={( e ) => e.stopPropagation()}>
				...
				<form action={handleSubmit}>
					<div className='mt-4 grid grid-cols-3'>
						<label className="text-right p-2">Title: </label> <input name={'name'} placeholder="title of the todo" className="text-black/80 px-2 m-1 col-span-2"/>
						...
						<label className="text-right p-2">Deadline: </label> <div className="p-1 flex col-span-2"><div className="mt-1.5"><Switch value={hasDeadline.value} size='tiny' onToggle={hasDeadline.toggle} /></div> {hasDeadline.value && ( <input className='px-1 ml-5 w-32 text-black/80' type='date' name='deadline'/> )}</div>
					</div>
