...
export const DoneActivitiesHistory = ( {doneActivities}: IProps ) => {
	const [optimisticActivities, deleteOptimisticActivity] = useOptimistic<IDoneActivity[], number>(
		doneActivities,
		( state: IDoneActivity[], deletedActivity: number ) => {
	...
	const days = dummyArray.map( ( day, i ) => {
		...
		return {
			...
			activities: optimisticActivities.filter( ( activity ) => isSameDay( activity.created_at, thisDay ) )
		};
	} );
	return (
